...
local function progressBar(func)
    exports['progressbar']:Progress("takeout_vehicle", 'Pulling out vehicle', math.random(3000, 5000), false, true, {
        ...
        if hit then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                if dis < 2.5 then
                    ...
                    ESX.ShowNotification(locale('cant_place'), 3, 3000)
                end
            end
        end
        Wait(1)
...
local function openVehiclesMenu(type, depot)
    ...
    if not next(vehicles) then return ESX.ShowNotification(locale('no_vehicles'), 2, 3000) end
    ...
    for _, car in pairs(vehicles) do
        local mods = json.decode(car.vehicle)

        vehicleMenu.options[#vehicleMenu.options + 1] = {
            title = GetVehicleName(mods.model),
            ...
            onSelect = function()
                if type == 'impound' then
                    ...
                    TriggerServerEvent('void_parking:server:vehicleTakeout', mods)
                end
            end,
        }
    end
...
local function storeVehicle()
    ...
    if not vehicle then return ESX.ShowNotification(locale('vehicle_not_found'), 2, 3000) end
...
lib.callback.register('void_parking:getAvailableSpot', function(positions)
    for k, v in pairs(positions) do
        if not IsPositionOccupied(v.x, v.y, v.z, 5, false, true, true) then
            ESX.ShowNotification(locale('depot_take_out'), 1, 3000)
            return v
        end
    end
    ESX.ShowNotification(locale('depot_err_no_space'), 2, 5000)
end)
...
function GetVehicleName(model)
    local encode = json.encode(model)
    local displayName = GetLabelText(GetDisplayNameFromVehicleModel(tonumber(encode)):lower())
