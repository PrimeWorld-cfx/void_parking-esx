...
RegisterNetEvent('void_parking:server:placeMeter', function(coords, rotation)
    ...
    exports.ox_inventory:RemoveItem(player, 'parking_meter', 1, nil, true)
...
RegisterNetEvent('void_parking:server:storeVehicle', function(vehicle, vehMods, hours)
    ...
    if not vehicle then return TriggerClientEvent('esx:showNotification', src, locale('vehicle_not_found'), 3, 3000) end

    local plate = GetVehicleNumberPlateText(vehicle)
    local vehCoords = GetEntityCoords(vehicle)

    if #(pedCoords - vehCoords) > 7 then return TriggerClientEvent('esx:showNotification', src, locale('too_far'), 3, 3000) end
    ...
    TriggerClientEvent('esx:showNotification', src, locale('vehicle_stored'), 1, 4000)
end)
...
RegisterNetEvent('void_parking:server:vehicleTakeout', function(vehicle)
    ...
    Entity(veh).state:set('vehicleProperties', vehicle.vehicle, true)
end)
...
lib.callback.register('void_parking:server:getVehicles', function(source, type)
    ...
    if type == 'impound' then
        ...
        vehicles = db.getVehicles(identifier)
    end
...
RegisterNetEvent('void_parking:server:takeOutImpound', function(data)
    ...
    local vehicleMods = json.decode(data.vehicleData.vehicle)
...
if Config.Inventory ~= 'ox_inventory' then
    ESX.RegisterUsableItem('parking_meter', function(source, item)
...
RegisterCommand('parkingsetup', function(source)
    ...
    MySQL.query('UPDATE owned_vehicles SET state = 1 WHERE state = 0;')
